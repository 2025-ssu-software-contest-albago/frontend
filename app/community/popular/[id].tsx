...
export default function PopularPostDetailScreen() {
  ...
  // 날짜 포매팅 함수
  const formatDate = (dateStr: string | number | Date | undefined | null) => {
    ...
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>인기 글 목록</Text>
        </View>
          ...
          <View style={styles.actionRow}>
            <TouchableOpacity style={styles.iconRow} onPress={handleLike}>
              <AntDesign
                ...
                color="#e53935"
              />
              <Text style={styles.count}>{post.likes + (postLiked ? 1 : 0)}</Text>
            </TouchableOpacity>
            ...
            {comments.map((comment) => (
              <View key={comment.id} style={styles.commentItem}>
                ...
                <View style={styles.commentActions}>
                  ...
                  >
                    <AntDesign
                      name={commentLikes[comment.id] ? "heart" : "hearto"}
                      size={14}
                      color="#e53935"
                    />
                    <Text style={styles.commentCount}>{comment.likes || 0}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    onPress={() => router.push({
