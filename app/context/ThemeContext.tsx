...
export function PostProvider({ children }: { children: ReactNode }) {
  ...
  return (
    ...
    >
      {children}
    </PostContext.Provider>stContext.Provider>
  );
}

// 커스텀 훅으로 Context 사용 편리하게스텀 훅으로 Context 사용 편리하게
export function usePostContext() {xport function usePostContext() {
  const context = useContext(PostContext);  const context = useContext(PostContext);
  if (context === undefined) {d) {
    throw new Error('usePostContext must be used within a PostProvider');t must be used within a PostProvider');
  }
  return context;
}